...
from models.fastwam.dit import DiTBlock, VideoDiT, ActionDiT, apply_rope
...
class FastWAMMoT(nn.Module):
    ...
    def preprocess_expert_attention_inputs(
        ...
        msa_shift, msa_scale, msa_gate, mlp_shift, mlp_scale, mlp_gate = block.split_modulation(time_embeds)
    ...
    def prefill_video_cache(
        ...
        for layer_index, video_block in enumerate(self.mixtures["video"].blocks):
            ...
            # [B,H,L,C] -> [B,L,C]
            attn = attn.transpose(1, 2).reshape(attn.shape[0], attn.shape[2], video_block.self_attn.attn_dim)
    ...
    def forward_with_video_cache(
        ...
        action_attn_mask = None if attn_mask is None else attn_mask[video_seq_len:seq_len, :seq_len]
        ...
        for layer_index, action_block in enumerate(self.mixtures["action"].blocks):
            ...
            # [B,H,L,C] -> [B,L,C]
            attn = attn.transpose(1, 2).reshape(attn.shape[0], action_seq_len, action_block.self_attn.attn_dim)
    ...
    def forward(
        ...
        for layer_index, (video_block, action_block) in enumerate(
            ...
            query = torch.cat([video_attn_inputs.query, action_attn_inputs.query], dim=2)
            key = torch.cat([video_attn_inputs.key, action_attn_inputs.key], dim=2)
            value = torch.cat([video_attn_inputs.value, action_attn_inputs.value], dim=2)
            ...
            # [B,H,L,C] -> [B,L,C]
            attn = attn.transpose(1, 2).reshape(batch_size, video_seq_len + action_seq_len, attn_dim)
