...
def get_freqs_cis1d_cache(dim: int, end: int = 1024, base: float = 10000.0) -> torch.Tensor:
...
def sin_embeds1d(dim: int, position: torch.Tensor, base: float = 10000.0) -> torch.Tensor:
    ...
    sinusoid = torch.outer(
        ...
        torch.pow(base, -torch.arange(dim // 2, device=position.device, dtype=torch.float64) / (dim // 2)),
    )
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, hidden_dim: int, head_dim: int, num_heads: int, eps: float = 1e-6) -> None:
        super().__init__()
...
class DiTBlock(nn.Module):
    ...
    def modulate(self, x: torch.Tensor, scale: torch.Tensor, shift: torch.Tensor) -> torch.Tensor:
    ...
    def split_modulation(self, time_embeds: torch.Tensor) -> tuple[torch.Tensor, ...]:
        ...
        chunks = (self.modulation.to(device=time_embeds.device, dtype=time_embeds.dtype) + time_embeds).chunk(
            6, dim=chunk_dim
        )
    ...
    def forward(
        ...
        Input:
            ...
            time_embeds: [B,L,6,C]
        """
        msa_shift, msa_scale, msa_gate, mlp_shift, mlp_scale, mlp_gate = self.split_modulation(time_embeds)
...
class Head(nn.Module):
    ...
    def forward(self, x: torch.Tensor, time_embeds: torch.Tensor) -> torch.Tensor:
        ...
        shift, scale = (
            self.modulation.unsqueeze(0).to(device=time_embeds.device, dtype=time_embeds.dtype)
...
class VideoDiT(CheckpointModule, nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [DiTBlock(hidden_dim, head_dim, num_heads, ffn_dim, eps=eps) for _ in range(num_blocks)]
        )
    ...
    def build_self_attn_mask(
        ...
        if self.self_attn_mask_mode == "bidirectional":
            return torch.ones((video_seq_len, video_seq_len), device=device, dtype=torch.bool)
    ...
    def preprocess(
        ...
        timestep = torch.ones((B, T, tokens_per_frame), device=video_tokens.device) * timestep.view(B, 1, 1)
        ...
        time_projs = self.time_projection(time_embeds).unflatten(2, (6, self.hidden_dim))
...
class ActionDiT(CheckpointModule, nn.Module):
    ...
    def preprocess(
        ...
        time_projs = self.time_projection(time_embeds).unflatten(1, (6, self.hidden_dim))
