...
class CausalConv3D(nn.Conv3d):
    ...
    def forward(self, x: torch.Tensor, feature_cache: Optional[torch.Tensor] = None) -> torch.Tensor:
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_dim: int, out_dim: int, dropout: float = 0.0) -> None:
        ...
        self.shortcut = (
            CausalConv3D(self.in_dim, self.out_dim, kernel_size=1) if self.in_dim != self.out_dim else nn.Identity()
        )
...
class Resample(nn.Module):
    ...
    def __init__(
        ...
        assert mode in self.MODE, f"Resample mode must be one of [{self.MODE}], got `{mode}`."
        super().__init__()
    ...
    def forward(
        ...
        if self.mode == "downsample3d" and feature_cache is not None:
            ...
            if feature_cache[idx] is None:
                ...
                cache_x = x[:, :, -1:, :, :].clone()
                x = self.time_conv(torch.cat([feature_cache[idx][:, :, -1:, :, :], x], dim=2))
                feature_cache[idx] = cache_x
                feature_idx[0] += 1
...
class DownResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_res_blocks):
            downsamples.append(ResidualBlock(in_dim=in_dim, out_dim=out_dim, dropout=dropout))
            in_dim = out_dim
...
class VAEEncoder3D(nn.Module):
    ...
    def __init__(
        ...
        for i, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            t_downsample = self.temporal_downsample[i] if i < len(self.temporal_downsample) else False
        ...
        self.head = nn.Sequential(
            ...
            nn.SiLU(),
            CausalConv3D(in_channels=out_dim, out_channels=z_dim, kernel_size=3, padding=1),
        )
...
class VAEDecoder3D(nn.Module):
    ...
    def __init__(
        ...
        upsamples = []
        for i, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            t_upsample = self.temporal_upsample[i] if i < len(self.temporal_upsample) else False
        ...
        self.head = nn.Sequential(
            ...
            nn.SiLU(),
            CausalConv3D(in_channels=out_dim, out_channels=12, kernel_size=3, padding=1),
        )
...
class VAEModel(nn.Module):
    ...
    def encode(self, x: torch.Tensor, scales: list[torch.Tensor]) -> torch.Tensor:
        ...
        mu = (mu - scales[0].view(1, self.z_dim, 1, 1, 1)) * scales[1].view(1, self.z_dim, 1, 1, 1)
    ...
    def decode(self, x: torch.Tensor, scales: list[torch.Tensor]) -> torch.Tensor:
        ...
        x = x / scales[1].view(1, self.z_dim, 1, 1, 1) + scales[0].view(1, self.z_dim, 1, 1, 1)
...
class WanVideoVAE(ModelMixin, ConfigMixin):
    ...
    def single_decode(self, hidden_states: torch.Tensor, device: torch.device) -> torch.Tensor:
        hidden_states = hidden_states.to(device)
        video = self.model.decode(hidden_states, self.scales)
        return video.clamp_(-1, 1)

    def encode(self, videos: list[torch.Tensor] | torch.Tensor, device: torch.device) -> torch.Tensor:
        hidden_states = []
        for video in videos:
    ...
    def decode(self, hidden_states: list[torch.Tensor] | torch.Tensor, device: torch.device) -> torch.Tensor:
        videos = []
